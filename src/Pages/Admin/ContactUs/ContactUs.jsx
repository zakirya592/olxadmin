...
import {AdminaboutColumn} from "../../../../utils/Datatablesource";
...
const ContactUs = () => {
  ...
  const handleShowUpdatePopup = (row) => {
    ...
    sessionStorage.setItem("updateaContactUs", JSON.stringify(row));
  };
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const response = await NewRequest.get("/about/contact");
  ...
  const handleDelete = async (row) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const isDeleted = await NewRequest.delete("about/contact/" + row?._id);
          if (isDeleted) {
            toast.success(`Contact Us has been deleted successfully!`);
          ...
          <div className="h-auto w-[97%] px-0 pt-4">
            <div className="h-auto w-full p-4 bg-white shadow-xl rounded-md">
              <div className={`flex px-3 flex-row justify-start`}>
                ...
                >
                  <i className="fas fa-plus mr-2"></i>Add Contact Us
                </button>
              </div>
