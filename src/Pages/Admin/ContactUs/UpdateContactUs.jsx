...
const UpdateContactUs = ({ isVisible, setVisibility, refreshBrandData }) => {
  const updateBrandData = JSON.parse(sessionStorage.getItem("updateaContactUs"));
  const [name, setname] = useState(updateBrandData?.caption || "");
  ...
  const handleAddCompany = async () => {
    try {
      const response = await NewRequest.put(`/about/contact/${updateBrandData._id}`, {
        caption: name,
      });
      console.log(response);
      toast.success(`ContactUs has been update successfully".`, {
        position: "top-right",
        autoClose: 2000,
